class FooterLinks extends HTMLElement {
  connectedCallback() {
    ...
    if (this.getAttribute("addLinks")) {
      ...
      previewLinks = defaultLinks.filter((link) =>
        addIcons.includes(link.name),
      );
    } else {
    ...
    if (this.getAttribute("removeLinks")) {
      ...
      previewLinks = previewLinks.filter(
...

    {
      ...
      let previewLinks1 = this.getAttribute("addLinks")
        ? this.getAttribute("addLinks").split("")
        : defaultLinks;

      let removeLinks1 = this.getAttribute("removeIcons")
        ? this.getAttribute("removeIcons").split(" ")
        : [];

      previewLinks1 = previewLinks1.filter(
        (links) => !removeLinks1.includes(links.name),
      );
    }

    // Generate the markup for the icons
    const linksMarkup = previewLinks
      .map((link) => `<a href="${link.url}"><p>${link.name}</p></a>`)
      .join("");
