class FooterIcons extends HTMLElement {
	connectedCallback() {
		...
		if (this.getAttribute("addIcons")) {
			...
			previewIcons = defaultIcons.filter((icon) => addIcons.includes(icon.name));
		} else {
		...
		if (this.getAttribute("removeIcons")) {
			...
			previewIcons = previewIcons.filter((icon) => !removeIcons.includes(icon.name));
		}

		{
			// the above whole process can be written simply
			let previewIcons1 = this.getAttribute("addIcons") ? this.getAttribute("addIcons").split("") : defaultIcons;

			let removeIcons1 = this.getAttribute("removeIcons") ? this.getAttribute("removeIcons").split(" ") : [];

			previewIcons1 = previewIcons1.filter((icon) => !removeIcons1.includes(icon.name));
		}
